...
for (fname,elty) in ((:rocsparse_sbsrmm, :Float32),
    ...
    @eval begin
        function mm!(
            ...
            A::ROCSparseMatrixBSR{$elty}, B::StridedROCMatrix{$elty},
            beta::Number, C::StridedROCMatrix{$elty}, index::SparseChar,
        )
...
for (fname,elty) in ((:rocsparse_scsrmm, :Float32),
    ...
    @eval begin
        function mm2!(
            transa::SparseChar, transb::SparseChar, alpha::Number,
            A::ROCSparseMatrixCSR{$elty}, B::StridedROCMatrix{$elty},
            beta::Number, C::StridedROCMatrix{$elty}, index::SparseChar,
        )
            if transb == 'C'
        ...
        function mm2!(
            transa::SparseChar, transb::SparseChar, alpha::Number,
            A::ROCSparseMatrixCSC{$elty}, B::StridedROCMatrix{$elty},
            beta::Number, C::StridedROCMatrix{$elty}, index::SparseChar,
        )
            if transb == 'C'
                throw(ArgumentError("B^H is not supported"))
...
"""
sm2!(
    transa::SparseChar, transxy::SparseChar, diag::SparseChar,
    alpha::Number, A::ROCSparseMatrix, X::ROCMatrix, index::SparseChar)

# bsrsm2
for (bname,aname,sname,elty) in (
    (:rocsparse_sbsrsm_buffer_size, :rocsparse_sbsrsm_analysis, :rocsparse_sbsrsm_solve, :Float32),
