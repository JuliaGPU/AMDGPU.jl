...
struct ROCSparseVecStyle <: Broadcast.AbstractArrayStyle{1} end
struct ROCSparseMatStyle <: Broadcast.AbstractArrayStyle{2} end
Broadcast.BroadcastStyle(::Type{<:ROCSparseVector}) = ROCSparseVecStyle()
Broadcast.BroadcastStyle(::Type{<:ROCSparseMatrix}) = ROCSparseMatStyle()
const SPVM = Union{ROCSparseVecStyle,ROCSparseMatStyle}

# ROCSparseVecStyle handles 0-1 dimensions, ROCSparseMatStyle 0-2 dimensions.
# ROCSparseVecStyle promotes to ROCSparseMatStyle for 2 dimensions.
# Fall back to DefaultArrayStyle for higher dimensionality.
ROCSparseVecStyle(::Val{0}) = ROCSparseVecStyle()
ROCSparseVecStyle(::Val{1}) = ROCSparseVecStyle()
ROCSparseVecStyle(::Val{2}) = ROCSparseMatStyle()
ROCSparseVecStyle(::Val{N}) where N = Broadcast.DefaultArrayStyle{N}()
ROCSparseMatStyle(::Val{0}) = ROCSparseMatStyle()
ROCSparseMatStyle(::Val{1}) = ROCSparseMatStyle()
ROCSparseMatStyle(::Val{2}) = ROCSparseMatStyle()
ROCSparseMatStyle(::Val{N}) where N = Broadcast.DefaultArrayStyle{N}()

Broadcast.BroadcastStyle(::ROCSparseVecStyle, ::ROCArrayStyle{1}) = ROCSparseVecStyle()
Broadcast.BroadcastStyle(::ROCSparseVecStyle, ::ROCArrayStyle{2}) = ROCSparseMatStyle()
Broadcast.BroadcastStyle(::ROCSparseMatStyle, ::ROCArrayStyle{2}) = ROCSparseMatStyle()
...
@inline capturescalars(f, mixedargs::Tuple{Ref{Type{T}}, Ref{Type{S}}, Vararg{Any}}) where {T, S} =
    ...
    capturescalars((args...)->f(T, args...), Base.tail(mixedargs))
@inline capturescalars(f, mixedargs::Tuple{ROCSparseVecOrMat, Ref{Type{T}}, Vararg{Any}}) where {T} =
...
function CSRIterator{Ti}(row, args::Vararg{<:Any, N}) where {Ti,N}
    ...
    @boundscheck begin
        ntuple(Val(N)) do i
            arg = @inbounds args[i]
            arg isa ROCSparseDeviceMatrixCSR && checkbounds(arg, row, 1)
        end
    end
...
function CSCIterator{Ti}(col, args::Vararg{<:Any, N}) where {Ti,N}
    ...
    @boundscheck begin
        ntuple(Val(N)) do i
            arg = @inbounds args[i]
            arg isa ROCSparseDeviceMatrixCSR && checkbounds(arg, 1, col)
        end
    end
...
@inline function Base.iterate(iter::CSCIterator{Ti,N}, state=nothing) where {Ti,N}
    ...
    @inline function get_col(i, ptr)
        arg = @inbounds iter.args[i]
        if arg isa ROCSparseDeviceMatrixCSC
...
function _getindex(arg::Union{ROCSparseDeviceMatrixCSR,ROCSparseDeviceMatrixCSC}, I, ptr)
    if ptr == 0
        zero(eltype(arg))
    else
...
function Broadcast.copy(bc::Broadcasted{<:Union{ROCSparseVecStyle,ROCSparseMatStyle}})
    ...
    if !fpreszeros
        ...
        output = ROCArray{Tv}(undef, size(bc))
        ...
        nonsparse_args = map(bc.args) do arg
            ...
            if arg isa AbstractROCSparseArray
                zero(eltype(arg))
            else
                arg
            end
    ...
    elseif length(sparse_args) == 1
        ...
        if sparse_typ <: ROCSparseMatrixCSR
            ...
            output = ROCSparseMatrixCSR(rowPtr, colVal, nzVal, size(bc))
        elseif sparse_typ <: ROCSparseMatrixCSC
            offsets = colPtr = sparse_arg.colPtr
            rowVal = similar(sparse_arg.rowVal)
            nzVal = similar(sparse_arg.nzVal, Tv)
            output = ROCSparseMatrixCSC(colPtr, rowVal, nzVal, size(bc))
        end
        ...
        offsets = if sparse_typ <: ROCSparseMatrixCSR
            ROCArray{Ti}(undef, rows+1)
        elseif sparse_typ <: ROCSparseMatrixCSC
            ROCArray{Ti}(undef, cols+1)
        end
        let
        ...
        output = if sparse_typ <: ROCSparseMatrixCSR
            colVal = ROCArray{Ti}(undef, total_nnz)
            nzVal = ROCArray{Tv}(undef, total_nnz)
            ROCSparseMatrixCSR(offsets, colVal, nzVal, size(bc))
        elseif sparse_typ <: ROCSparseMatrixCSC
            rowVal = ROCArray{Ti}(undef, total_nnz)
            nzVal = ROCArray{Tv}(undef, total_nnz)
            ROCSparseMatrixCSC(offsets, rowVal, nzVal, size(bc))
        end
    end
