...
# space.
@generated function get_global_pointer(::Val{global_name}, ::Type{T})::AMDGPU.DevicePtr{T} where {global_name, T}
    JuliaContext() do ctx
        ...
        # Call the function.
        quote
            AMDGPU.DevicePtr{T, AMDGPU.AS.Global}(convert(Csize_t, $(call_function(llvm_f, Ptr{T}))))
        end
    end
end
