...
# math.jl
@device_override @inline Base.Math.throw_complex_domainerror(f::Symbol, x) =
    @gpu_throw "DomainError: This operation requires a complex input to return a complex result"
@device_override @inline Base.Math.throw_exp_domainerror(f::Symbol, x) =
    @gpu_throw "DomainError: Exponentiation yielding a complex result requires a complex argument"

# intfuncs.jl
@device_override @inline Base.throw_domerr_powbysq(::Any, p) =
    @gpu_throw "DomainError: Cannot raise an integer to a negative power"
@device_override @inline Base.throw_domerr_powbysq(::Integer, p) =
    @gpu_throw "DomainError: Cannot raise an integer to a negative power"
@device_override @inline Base.throw_domerr_powbysq(::AbstractMatrix, p) =
    @gpu_throw "DomainError: Cannot raise an integer to a negative power"
@device_override @inline Base.__throw_gcd_overflow(a, b) =
    @gpu_throw "OverflowError: GCD overflow"

# checked.jl
@device_override @inline Base.Checked.throw_overflowerr_binaryop(op, x, y) =
    @gpu_throw "OverflowError: Binary operation overflowed"
@device_override @inline Base.Checked.throw_overflowerr_negation(op, x, y) =
    @gpu_throw "OverflowError: Negation overflowed"
@device_override @inline function Base.Checked.checked_abs(x::Base.Checked.SignedInt)
    r = ifelse(x < 0, -x, x)
