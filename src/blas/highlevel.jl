...
#= FIXME
Base.argmin(xs::ROCArray{T}) where T<:ROCBLASFloat = iamin(xs)
Base.argmax(xs::ROCArray{T}) where T<:ROCBLASFloat = iamax(xs)
=#
...
LinearAlgebra.mul!(Y::ROCVector{T}, A::LinearAlgebra.Transpose{T, ROCMatrix{T}}, B::ROCVector{T}) where T<:ROCBLASFloat = gemv_wrapper!(Y, 'T', A.parent, B)
LinearAlgebra.mul!(Y::ROCVector{T}, A::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, B::ROCVector{T}) where T<:ROCBLASReal = gemv_wrapper!(Y, 'T', A.parent, B)
LinearAlgebra.mul!(Y::ROCVector{T}, A::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, B::ROCVector{T}) where T<:ROCBLASComplex = gemv_wrapper!(Y, 'C', A.parent, B)
...
LinearAlgebra.mul!(C::ROCMatrix{T}, trA::LinearAlgebra.Transpose{T, ROCMatrix{T}}, B::ROCMatrix{T}) where T<:ROCBLASFloat =
    gemm_wrapper!(C, 'T', 'N', parent(trA), B)
LinearAlgebra.mul!(C::ROCMatrix{T}, A::ROCMatrix{T}, trB::LinearAlgebra.Transpose{T, ROCMatrix{T}}) where T<:ROCBLASFloat =
    gemm_wrapper!(C, 'N', 'T', A, parent(trB))
LinearAlgebra.mul!(C::ROCMatrix{T}, trA::LinearAlgebra.Transpose{T, ROCMatrix{T}}, trB::LinearAlgebra.Transpose{T, ROCMatrix{T}}) where T<:ROCBLASFloat =
    gemm_wrapper!(C, 'T', 'T', parent(trA), parent(trB))
LinearAlgebra.mul!(C::ROCMatrix{T}, adjA::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, B::ROCMatrix{T}) where T<:ROCBLASReal =
    gemm_wrapper!(C, 'T', 'N', parent(adjA), B)
LinearAlgebra.mul!(C::ROCMatrix{T}, adjA::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, B::ROCMatrix{T}) where T<:ROCBLASComplex =
    gemm_wrapper!(C, 'C', 'N', parent(adjA), B)
LinearAlgebra.mul!(C::ROCMatrix{T}, A::ROCMatrix{T}, adjB::LinearAlgebra.Adjoint{T, ROCMatrix{T}}) where T<:ROCBLASReal =
    gemm_wrapper!(C, 'N', 'T', A, parent(adjB))
LinearAlgebra.mul!(C::ROCMatrix{T}, A::ROCMatrix{T}, adjB::LinearAlgebra.Adjoint{T, ROCMatrix{T}}) where T<:ROCBLASComplex =
    gemm_wrapper!(C, 'N', 'C', A, parent(adjB))
LinearAlgebra.mul!(C::ROCMatrix{T}, adjA::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, adjB::LinearAlgebra.Adjoint{T, ROCMatrix{T}}) where T<:ROCBLASReal =
    gemm_wrapper!(C, 'T', 'T', parent(adjA), parent(adjB))
LinearAlgebra.mul!(C::ROCMatrix{T}, adjA::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, adjB::LinearAlgebra.Adjoint{T, ROCMatrix{T}}) where T<:ROCBLASComplex =
    gemm_wrapper!(C, 'C', 'C', parent(adjA), parent(adjB))
LinearAlgebra.mul!(C::ROCMatrix{T}, trA::LinearAlgebra.Transpose{T, ROCMatrix{T}}, adjB::LinearAlgebra.Adjoint{T, ROCMatrix{T}}) where T<:ROCBLASReal =
    gemm_wrapper!(C, 'T', 'T', parent(trA), parent(adjB))
LinearAlgebra.mul!(C::ROCMatrix{T}, trA::LinearAlgebra.Transpose{T, ROCMatrix{T}}, adjB::LinearAlgebra.Adjoint{T, ROCMatrix{T}}) where T<:ROCBLASComplex =
    gemm_wrapper!(C, 'T', 'C', parent(trA), parent(adjB))
LinearAlgebra.mul!(C::ROCMatrix{T}, adjA::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, trB::LinearAlgebra.Transpose{T, ROCMatrix{T}}) where T<:ROCBLASReal =
    gemm_wrapper!(C, 'T', 'T', parent(adjA), parent(trB))
LinearAlgebra.mul!(C::ROCMatrix{T}, adjA::LinearAlgebra.Adjoint{T, ROCMatrix{T}}, trB::LinearAlgebra.Transpose{T, ROCMatrix{T}}) where T <: ROCBLASComplex =
...
LinearAlgebra.ldiv!(A::Adjoint{T,UnitLowerTriangular{T,ROCMatrix{T}}}, B::ROCMatrix{T}) where T<:ROCBLASReal =
    rocBLAS.trsm!('L', 'L', 'T', 'U', one(T), parent(parent(A)), B)
LinearAlgebra.ldiv!(A::Adjoint{T,UpperTriangular{T,ROCMatrix{T}}}, B::ROCMatrix{T}) where T<:ROCBLASComplex =
    rocBLAS.trsm!('L', 'U', 'C', 'N', one(T), parent(parent(A)), B)
LinearAlgebra.ldiv!(A::Adjoint{T,UnitUpperTriangular{T,ROCMatrix{T}}}, B::ROCMatrix{T}) where T<:ROCBLASComplex =
    rocBLAS.trsm!('L', 'U', 'C', 'U', one(T), parent(parent(A)), B)
LinearAlgebra.ldiv!(A::Adjoint{T,LowerTriangular{T,ROCMatrix{T}}}, B::ROCMatrix{T}) where T<:ROCBLASComplex =
    rocBLAS.trsm!('L', 'L', 'C', 'N', one(T), parent(parent(A)), B)
LinearAlgebra.ldiv!(A::Adjoint{T,UnitLowerTriangular{T,ROCMatrix{T}}}, B::ROCMatrix{T}) where T<:ROCBLASComplex =
...
LinearAlgebra.rdiv!(A::ROCMatrix{T}, B::Adjoint{T,UnitLowerTriangular{T,ROCMatrix{T}}}) where T<:ROCBLASReal =
    rocBLAS.trsm!('R', 'L', 'T', 'U', one(T), parent(parent(B)), A)
LinearAlgebra.rdiv!(A::ROCMatrix{T}, B::Adjoint{T,UpperTriangular{T,ROCMatrix{T}}}) where T<:ROCBLASComplex =
    rocBLAS.trsm!('R', 'U', 'C', 'N', one(T), parent(parent(B)), A)
LinearAlgebra.rdiv!(A::ROCMatrix{T}, B::Adjoint{T,UnitUpperTriangular{T,ROCMatrix{T}}}) where T<:ROCBLASComplex =
    rocBLAS.trsm!('R', 'U', 'C', 'U', one(T), parent(parent(B)), A)
LinearAlgebra.rdiv!(A::ROCMatrix{T}, B::Adjoint{T,LowerTriangular{T,ROCMatrix{T}}}) where T<:ROCBLASComplex =
    rocBLAS.trsm!('R', 'L', 'C', 'N', one(T), parent(parent(B)), A)
LinearAlgebra.rdiv!(A::ROCMatrix{T}, B::Adjoint{T,UnitLowerTriangular{T,ROCMatrix{T}}}) where T<:ROCBLASComplex =
