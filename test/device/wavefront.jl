@testset "Wavefront Shuffle" begin
    ...
    function ker_shfl!(x, ::Type{T}) where T
        i = AMDGPU.Device.activelane()
        x[i + 1] = AMDGPU.Device.shfl(T(i), i + 1)
        return
    end

    # Same as above, but fill both high and low bits.
    function ker_shfl_composed!(x, ::Type{T}) where T
        i = AMDGPU.Device.activelane()
        v = T(i) << (sizeof(T) * 4) | T(i)
        x[i + 1] = AMDGPU.Device.shfl(v, i + 1)
        return
    end

    # Same as the first kernel, but instead test for even lane id.
    function ker_shfl_bool!(x)
        i = AMDGPU.Device.activelane()
        x[i + 1] = AMDGPU.Device.shfl(i % 2 == 0, i + 1)
        return
    end
    ...
    @testset "shfl composed T=$T" for T in (UInt64, UInt128, Int64, Int128)
        ...
        x = ROCArray{T}(undef, ws)
        @roc groupsize=ws ker_shfl_composed!(x, T)
        @test Array(x) == target
    end
    ...
    @testset "shfl boolean" begin
        ...
        @roc groupsize=ws ker_shfl_bool!(x)
        @test Array(x) == target
    end
end
